...
이 프로젝트는 집을 나서기 전에 핸드폰 앱으로 버스 정보를 확인하는 번거로움을 줄이기 위해 시작되었습니다. 이를 위해, 버스의 예상 도착 시간과 현재 위치를 실시간으로 알려주는 디바이스를 제작하였습니다. 따라서 사용자는 앱을 확인하지 않고도 바로 버스 정보를 확인하고 출발할 수 있게 되었습니다.

**주요 기능:**

1. **예상 도착 시간 및 위치 확인**: 버스의 예상 도착 시간과 현재 몇 정거장 전에 있는지 확인 가능.
2. **다음 버스 정보 확인**: 버튼을 누르면 다음 버스 정보를 표시.
3. **트래픽 제한 대응**: 지정된 시간(오전 6시부터 저녁 8시까지)에만 버스 정보 제공. 그 외 시간에는 'Unavailable time' 메시지와 현재 시간 표시.
...
5. 시간 주소의 경우 `https` 를 사용해야 합니다. `http`를 사용할 경우 301 오류 발생..
